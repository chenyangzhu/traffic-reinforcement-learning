    ...
    def execute(self, strategy, Y):
        ...
        for i in range(4):  # 对每个crossroad 从左边的entry开始是entry1 1234
            if strategy[i][0] == 1:
                ...
                outgoing cars= delta t(1s) * Vij / physical length of a car(3m)
                '''
                Z[i][1] = int(self.dt * self.V[i+1][0][1] / self.car_len)  # crossroad i  entry 1 to 2
                Z[i][3] = int(self.dt * self.V[i+1][2][3] / self.car_len)  # crossroad i  entry 3 to 4

            elif strategy[i][1] == 1:
                # 换成 elif 因为只有可能有一种状态发生。 --klaus [0 0 1]]

                Z[i][2] = int(self.dt * self.V[i+1][0][2] / self.car_len)  # crossroad i  entry 1 to 3
                Z[i][0] = int(self.dt * self.V[i+1][2][0] / self.car_len)  # crossroad i  entry 3 to 1

            elif strategy[i][2] == 1:
                Z[i][0] = int(self.dt * self.V[i+1][1][0] / self.car_len)  # crossroad i  entry 2 to 1
                Z[i][2] = int(self.dt * self.V[i+1][1][2] / self.car_len)  # crossroad i  entry 2 to 3
                Z[i][3] = int(self.dt * self.V[i+1][1][3] / self.car_len)  # crossroad i  entry 2 to 4
                Z[i][0] = int(self.dt * self.V[i+1][3][0] / self.car_len)  # crossroad i  entry 4 to 1
                Z[i][1] = int(self.dt * self.V[i+1][3][1] / self.car_len)  # crossroad i  entry 4 to 2
                Z[i][2] = int(self.dt * self.V[i+1][3][2] / self.car_len)  # crossroad i  entry 4 to 3
